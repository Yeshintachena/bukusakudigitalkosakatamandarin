...
{
  indonesia: "Coffee maker",
  mandarin: "咖啡机",
  pinyin: "kā fēi jī",
  category: "umum",
  direction: "id-cn",
  audio: "audio/ka-fei-ji.mp3",
  level: 2
},
{
...
{
  indonesia: "Mengevaluasi (menetapkan hasil penilaian)",
  mandarin: "评定",
  pinyin: "píng dìng",
  category: "umum",
  direction: "id-cn",
...
},
{
  ...
  mandarin: "保释手柄",
  pinyin: "bǎo shì shǒu bǐng",
  category: "umum",
  direction: "id-cn",
  audio: "audio/bao-shi-shou-bing.mp3",
  level: 4
},
{
  indonesia: "Pelanggan",
...
{
  indonesia: "Perencanaan jangka panjang (lebih terstruktur)",
  mandarin: "规划",
  pinyin: "guī huà",
  category: "umum",
  direction: "id-cn",
...
{
  indonesia: "Pisau atas berlapis karet (Six Blade Subassembly Om Assy)",
  mandarin: "上刀包胶",
  pinyin: "shàng dāo bāo jiāo",
  category: "umum",
  direction: "id-cn",
...
function setCategory(cat, el) {
  ...
  if (favTab) favTab.classList.remove("favorite-tab");
  renderList();
  ...
  if (favTab) {
    if (showFavoritesOnly) {
      ...
      document.querySelectorAll(".tab-btn").forEach(btn => {
        if (btn.id !== "favorites-tab") btn.classList.remove("active");
      });
    } else {
...
function renderList() {
  ...
  if (showFavoritesOnly) filtered = filtered.filter(item => favorites.includes(item.mandarin));
  if (filtered.length === 0) {
    list.innerHTML = `<div class="empty-state"><div class="empty-icon">😕</div><div class="empty-text">Waduh, tidak ditemukan!</div><div class="empty-suggestion">${showFavoritesOnly ? 'Belum ada favorit nih, klik ⭐ di kosakata' : 'Coba kata kunci lain atau pilih kategori berbeda'}</div></div>`;
    return;
  }
  filtered.forEach(item => {
    ...
    `;
    card.querySelector(".audio-btn").addEventListener("click", () => playAudio(item.audio));
